...
class SAC(DDPG):
    ...
    def build_actor(self, trainable=True):
        ...
        mean = self.dense_or_batchNorm(self.actionDim, "linear", use_bias=False, trainable=trainable, name="mean")(h)
        logStd = self.dense_or_batchNorm(self.actionDim, "linear", trainable=trainable, name="logStd")(h)
    ...
    def get_action_logProb(self, observ):
        mean, logStd = self.actor(observ)  # each (batchSz,actionDim)
        logStd = tf.clip_by_value(logStd, self.logit_min, self.logit_max)             # (batchSz,actionDim); to prevent gradient explosion
    ...
    def update_critic(self, observ, action, reward, next_observ, done, importance_weights):
        ...
        with tf.GradientTape(persistent=True) as tape:
            next_action, next_logProb = self.get_action_logProb(next_observ) # (batchSz,1)
